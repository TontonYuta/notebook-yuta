...
export function Sidebar({
  ...
  const handleDelete = (e: React.MouseEvent, id: string) => {
    ...
    if (
      window.confirm(
        "Bạn có chắc chắn muốn xóa quyển vở này? Toàn bộ nội dung sẽ bị mất.",
      )
    ) {
      onDelete(id);
    }
  ...
  const textNotebooks = useMemo(
    () => filteredNotebooks.filter((n) => n.type !== "pdf"),
    [filteredNotebooks],
  );
  const pdfNotebooks = useMemo(
    () => filteredNotebooks.filter((n) => n.type === "pdf"),
    [filteredNotebooks],
  );
      ...
      {(provided, snapshot) => (
        ...
        >
          ...
          <div
            ...
            style={{
              display:
                isSearching || editingId === notebook.id ? "none" : "block",
            }}
          ...
          {editingId === notebook.id ? (
            <div className="flex flex-col gap-2 relative z-10">
              ...
              >
                ...
                <button
                  type="submit"
                  className="text-[var(--color-green-pen)] cursor-pointer p-1"
                >
              ...
              {notebook.type !== "pdf" && (
                <div
                  className="flex gap-2 px-1 mb-1"
                  onClick={(e) => e.stopPropagation()}
                >
                  {COLORS.map((c) => (
                    <button
            ...
            <div className="flex items-center justify-between pointer-events-auto pl-4 relative z-10">
              <div className="flex items-center gap-2 flex-grow min-w-0 pr-2">
                {notebook.type === "pdf" ? (
                  <FileText
                    size={18}
                    className="text-[var(--color-red-pen)] shrink-0"
                    strokeWidth={2.5}
                  />
                ) : (
                  <div
                    className={`w-3 h-3 rounded-full border-2 shrink-0 ${COLOR_MAP[notebook.color || "default"]}`}
                  />
                )}
          ...
          <div className="text-xs opacity-50 mt-1 ml-9">
            Chỉnh sửa:{" "}
            {new Date(notebook.lastModified).toLocaleDateString("vi-VN")}
          </div>
        </div>
      )}
    </Draggable>
    ...
    <>
      ...
      <div
        className={`fixed md:relative top-0 left-0 h-full w-80 shrink-0 bg-[var(--color-cream)] border-r-2 border-[var(--color-ink)]/20 z-30 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] print:hidden shadow-[4px_0_15px_rgba(0,0,0,0.08)] flex flex-col ${
          isOpen
            ? "translate-x-0 ml-0"
            : "-translate-x-full md:translate-x-0 md:-ml-80"
        }`}
      >
        <div className="flex flex-col h-full bg-[var(--color-cream)] w-full absolute left-0">
          ...
          <div className="px-4 pt-4 pb-2">
            <div className="relative">
              ...
              />
              <Search
                className="absolute left-3 top-2.5 text-[var(--color-ink)]/50"
                size={18}
              />
            </div>
          </div>
          ...
          <DragDropContext onDragEnd={handleDragEnd}>
            <div className="flex-grow overflow-y-auto p-4 pt-2">
              <div className="mb-6">
                ...
                <Droppable
                  droppableId="notebooks-list"
                  isDropDisabled={isSearching}
                >
                  {(provided) => (
                    <div
                    ...
                    >
                      {textNotebooks.map((notebook, index) =>
                        renderDraggableItem(notebook, index),
                      )}
                ...
                >
                  {(provided) => (
                    <div
                      ...
                      {...provided.droppableProps}
                    >
                      {pdfNotebooks.map((notebook, index) =>
                        renderDraggableItem(notebook, index),
                      )}
                      {provided.placeholder}
                    </div>
          ...
          <div className="p-4 border-t-2 border-dashed border-[var(--color-ink)]/20 shrink-0 flex flex-col gap-3">
            ...
            >
              <Plus
                size={20}
                className="text-[var(--color-red-pen)]"
                strokeWidth={3}
              />
              Mua vở mới
            </button>
            <label className="w-full py-3 sketchy-border flex justify-center items-center gap-2 text-[var(--color-ink)] hover:bg-[var(--color-pastel-pink)]/30 transition-colors cursor-pointer font-bold notebook-body !text-lg bg-white relative">
              <Upload
                size={20}
                className="text-[var(--color-green-pen)]"
                strokeWidth={3}
              />
              Tải file lên
              <input
