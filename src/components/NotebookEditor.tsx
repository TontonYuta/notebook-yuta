...
export function NotebookEditor({
  ...
  const [localContent, setLocalContent] = useState(notebook?.content || "");
  const localContentRef = useRef(notebook?.content || "");

  useEffect(() => {
    if (notebook && notebook.content !== localContentRef.current) {
      setLocalContent(notebook.content);
      localContentRef.current = notebook.content;
    }
  }, [notebook?.id, notebook?.content]);
  ...
  const searchMatches = useMemo(() => {
    ...
    const text = deferredContent.toLowerCase();
    ...
    return matches;
  }, [deferredContent, notebook?.type, searchQuery]);
  ...
  const scrollToMatch = (
    ...
    if (index >= 0 && index < matches.length && textareaRef.current) {
      ...
      if (scroller) {
        const textUpToMatch = localContent.substring(0, matchPos) || "";
  ...
  const handleTextSelection = () => {
    ...
    if (textareaRef.current && document.activeElement === textareaRef.current) {
      ...
      if (
        ...
        text = localContent.substring(start, end);
        isFromTextarea = true;
      }
    }
...
"${selectedText}"`;
  ...
  const handlePrint = async () => {
    if (notebook.type === "pdf") {
      try {
        ...
        console.log(
          "Could not print iframe directly, opening the PDF in a new tab instead",
        );
      }
  ...
  return (
    <div
      className={`notebook-editor-container w-full h-full flex flex-col relative px-4 md:px-8 py-6 print:p-0 ${isPrinting ? "is-printing print:bg-white print:w-full print:block print:h-auto print:overflow-visible" : ""}`}
    >
        ...
        <div className="flex gap-3 justify-end items-center flex-wrap shrink-0">
          ...
          {notebook.type !== "pdf" && (
            <div className="flex items-center bg-white/60 backdrop-blur-sm rounded-full border-2 border-[var(--color-ink)] px-2 py-1 shadow-sm h-10 w-full sm:w-auto relative group focus-within:ring-2 ring-[var(--color-pastel-blue)]">
              <Search
                size={16}
                className="text-[var(--color-ink)] mr-2 flex-shrink-0"
              />
              <input
                type="text"
                value={searchQuery}
              ...
              {searchQuery && (
                <div className="flex items-center ml-2 border-l-2 border-[var(--color-ink)]/20 pl-2 gap-1 h-full">
                  ...
                  <button
                    onClick={clearSearch}
                    className="p-1 hover:bg-[var(--color-red-pen)]/20 text-[var(--color-red-pen)] rounded cursor-pointer ml-1"
                  >
          ...
          {notebook.type !== "pdf" && (
            <button
              ...
              title={
                showLines
                  ? "Tắt tự động căn dòng kẻ"
                  : "Bật tự động căn dòng kẻ"
              }
            >
          ...
          >
            <StickyIcon
              size={20}
              className="fill-current text-[var(--color-ink)] opacity-40"
              strokeWidth={2}
            />
          </button>
          ...
          {notebook.type !== "pdf" && (
            <div className="flex bg-white/60 backdrop-blur-sm rounded-full border-2 border-[var(--color-ink)] overflow-hidden shadow-sm font-bold text-[var(--color-ink)] h-10">
              <button
                ...
                title="Chế độ Viết"
              >
                <Edit3 size={16} strokeWidth={2.5} className="md:mr-1" />{" "}
                <span className="hidden md:inline">Viết</span>
              </button>
              <button
              ...
              >
                <Columns size={16} strokeWidth={2.5} className="md:mr-1" />{" "}
                <span className="hidden md:inline">Chia đôi</span>
              </button>
              <button
                ...
                title="Chế độ Xem"
              >
                <Eye size={16} strokeWidth={2.5} className="md:mr-1" />{" "}
      ...
      {isPrinting && (
        <div className="fixed inset-0 z-[100] bg-white/80 backdrop-blur-sm flex items-center justify-center no-print">
          <div className="bg-white border-2 border-[var(--color-ink)] p-6 rounded-2xl shadow-[8px_8px_0px_var(--color-ink)] flex flex-col items-center gap-4">
            ...
            <h3 className="text-xl font-bold font-heading text-[var(--color-ink)]">
              Đang tạo bản scan PDF...
            </h3>
            <p className="text-gray-500 text-sm">
              Vui lòng chờ một chút để ứng dụng chụp lại quyển vở.
            </p>
          </div>
        </div>
      )}
      ...
      <div
        ...
        className="w-full flex-grow flex gap-6 relative overflow-hidden print:overflow-visible print:block"
      >
        {/* Editor Half */}
        {notebook.type !== "pdf" &&
          ...
          >
            ...
            >
              {notebook.type === "pdf" ? (
                ...
                <div
                  className={`notebook-body bg-transparent ${showLines ? "lined-paper-preview" : ""} prose-notebook pl-[60px] pr-8 pt-8 pb-[50vh] min-h-full h-fit flex flex-col min-w-0 relative z-0 print:h-auto print:min-h-0 print:p-0`}
                >
      ...
      {/* Hint Modal */}
      {showHint && <CheatSheetModal onClose={() => setShowHint(false)} />}
    </div>
  );
}
